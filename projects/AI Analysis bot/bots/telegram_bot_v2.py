...
def _format_backtest_sparkline(net_r: float, max_dd_r: float, n_bars: int = 30) -> str:
    """Build a stylized ASCII equity curve from the headline aggregates.

    The shape is a labeled schematic, not a real trade-by-trade replay —
    we don't have the trade list in the JSON. The curve is a smooth
    interpolation from 0R to net_r over `n_bars` and dips by max_dd_r
    around the 30% mark so the eye sees the "down → up" shape that
    drove the headline numbers. The dip's amplitude is calibrated so
    it's visible relative to the linear ramp.
    """
    import math
    if net_r <= 0:
        return "(no equity)"
    # Each bar's running total: linear ramp minus a drawdown bump.
    dd_center = 0.30
    dd_width = 0.12
    # Scale the dip to be ~50% of the endpoint value so it's clearly
    # visible in the ASCII grid even after normalization. The label
    # line below the curve reports the actual max_dd_r number.
    dip_amp = max(max_dd_r, net_r * 0.4)
    bars = []
    for i in range(n_bars):
        progress = (i + 0.5) / n_bars
        ramp = progress * net_r
        # Gaussian-shaped dip
        dip = -dip_amp * math.exp(-((progress - dd_center) ** 2) / (2 * dd_width ** 2))
        bars.append(ramp + dip)
    # Normalize to a 0-7 height grid. The first char is "·" (middle
    # dot) so the lowest bar is still visible — a literal space would
    # hide the start of the curve and the dip would look invisible.
    lo, hi = min(bars), max(bars)
    span = hi - lo if hi > lo else 1
    grid = "·▁▂▃▄▅▆▇█"
    out = []
    for v in bars:
        idx = int((v - lo) / span * (len(grid) - 1))
        out.append(grid[idx])
    return "".join(out)


async def backtest_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/backtest — show the latest backtest results for the bot's strategy.

    Renders the headline aggregates from backtest_results_v2.json as a
    single card, plus an ASCII equity-curve schematic, a regime
    breakdown, and a 1-line verdict. The numbers come from a 90-day
    walk-forward over production `ReportBuilder.build()`.
    """
    if not update.message:
        return

    # Resolve the backtest JSON relative to the project root. The file
    # is regenerated by `python backtest_v2.py`; we read it on demand
    # so users always see the latest run.
    import json
    import os
    ...
    if not os.path.exists(json_path):
        await update.message.reply_text(
            "ℹ️ No backtest results found yet. Run `python backtest_v2.py` "
            "to generate the data, then try again.",
            reply_markup=get_main_menu_keyboard('free'),
        )
        return
    ...
    try:
        ...
        logger.error(f"backtest_command: failed to load {json_path}: {e}")
        await update.message.reply_text(
            "⚠️ Could not load backtest data right now. Try again in a moment.",
            reply_markup=get_main_menu_keyboard('free'),
        )
        return
    ...
    if not combos:
        await update.message.reply_text(
            "ℹ️ Backtest has no combos to display.",
            reply_markup=get_main_menu_keyboard('free'),
        )
        return

    # Use the first combo for the headline card. (V2 only ran one combo —
    # BTCUSDT H1 swing — but the code supports multiple.)
    c = combos[0]
    symbol = c.get('symbol', '?')
    timeframe = c.get('timeframe', '?')
    plan = c.get('plan', '?')
    n_tradable = c.get('n_tradable', 0)
    n_wins = c.get('n_wins', 0)
    n_losses = c.get('n_losses', 0)
    n_timeouts = c.get('n_timeouts', 0)
    win_rate = c.get('win_rate', 0.0)
    avg_win_r = c.get('avg_win_r', 0.0)
    avg_loss_r = c.get('avg_loss_r', 0.0)
...
"""
    ...
    if by_regime:
        ...
        sorted_regimes = sorted(
            by_regime.items(),
            key=lambda kv: kv[1][1] if len(kv[1]) > 1 else 0,
            reverse=True,
        )
        for regime, (count, wr) in sorted_regimes:
            bar_pct = wr / 100
            bar = "█" * int(bar_pct * 10) + "░" * (10 - int(bar_pct * 10))
            text += f"  {regime:20s} {wr:5.1f}%  {bar}  ({count})\n"
    ...
    if by_conf:
        ...
        for bucket, (count, wr) in by_conf.items():
            bar_pct = wr / 100
            bar = "█" * int(bar_pct * 10) + "░" * (10 - int(bar_pct * 10))
            text += f"  {bucket:8s} {wr:5.1f}%  {bar}  ({count})\n"

    # Verdict line — honest, not marketing. Cite the "look-ahead" PASS if
    # the canary block is present (means the backtest isn't cheating).
    canary = data.get('canary') or {}
    canary_line = ""
    if canary.get('verdict') == 'PASS':
        canary_line = "🔬 Look-ahead test: **PASS** (results not contaminated by future data)\n"
    ...
    if pf_ok and wr_ok:
        ...
        verdict = ("❌ **VERDICT: NOT VIABLE in backtest** — live order-flow + "
                   ...
                   "Use as a *floor* not a ceiling. Don't size up until you "
